  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Retire produtos do estoque</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-card rounded-2xl border border-border p-6 space-y-5">
        <div className="space-y-2">
          ...
          <ProductSelector
            ...
            filterFn={(p) => (p.quantity || 0) > 0}
          />
        </div>

        {selectedProduct && (
          <div className="px-4 py-3 rounded-xl bg-muted text-sm">
            Disponível: <span className="font-bold">{selectedProduct.quantity || 0}</span> {selectedProduct.unit || "un."}
          </div>
        )}
        ...
        >
          {createMutation.isPending ? "Criando..." : "Criar Pedido de Retirada"}
        </Button>
      </form>
    </div>
  );

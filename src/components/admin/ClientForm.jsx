...
export default function ClientForm({ client, onClose }) {
      ...
      if (client) {
        if (password) {
          try {
            ...
            if (!msg.includes("already") && !msg.includes("exist") && !msg.includes("registered")) throw err;
          }
        }
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!client && (!form.email || !form.password)) {
      toast.error("Informe o email e a senha de acesso do cliente.");
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4 pt-2">
      ...
      <div className="rounded-xl border border-border p-4 space-y-3 bg-muted/30">
        ...
        <div className="space-y-1.5">
          ...
          />
          {client && (
            <p className="text-xs text-muted-foreground">O email não pode ser alterado após a criação.</p>
          )}
        </div>
        <div className="space-y-1.5">
          <Label>{client ? "Nova Senha (opcional)" : "Senha *"}</Label>
          ...
          <p className="text-xs text-muted-foreground">
            O cliente acessa o sistema com este email e senha.
          </p>
        </div>
      </div>
